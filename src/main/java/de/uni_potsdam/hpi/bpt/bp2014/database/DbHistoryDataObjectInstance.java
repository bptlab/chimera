...
public class DbHistoryDataObjectInstance extends DbObject {
    ...
    public Map<Integer, Map<String, Object>> getLogEntriesForScenarioInstance(int scenarioInstanceId){
        //String sql = "SELECT * FROM historydataobjectinstance WHERE scenarioinstance_id = "+scenarioInstanceId+" ORDER BY timestamp DESC";
         //    "SELECT h.id, h.scenarioinstance_id, cn.label, h.activityinstance_id, h.oldstate, h.newstate, h.timestamp FROM historydataobjectinstance AS h, controlnode AS cn, controlnodeinstance AS cni WHERE h.scenarioinstance_id = "+scenarioInstanceId+"  AND h.activityinstance_id = cni.id AND cni.controlnode_id = cn.id  ORDER BY timestamp DESC";
        String sql = "";
