...
public class DbScenarioInstance extends DbObject {
	static Logger log = Logger.getLogger(DbScenarioInstance.class.getName());
	private boolean exist = false;
	private int scenario_id = -1;
	private int scenarioInstance_id = -1;
	...
	public Boolean existScenario(int scenarioInstance_id) {
		if (!exist || this.scenarioInstance_id != scenarioInstance_id) {
	...
	public LinkedList<Integer> getScenarioInstances(int scenario_id) {
		String sql =
				"SELECT * FROM scenarioinstance WHERE scenarioinstance.terminated = 0 AND scenario_id = "
						+ scenario_id;
		log.info(sql);
	...
	public void setTerminated(int scenarioInstance_id, boolean terminated) {
		...
		String sql = "UPDATE scenarioinstance SET scenarioinstance.terminated = " + terminatedAsInt
				+ " WHERE id = " + scenarioInstance_id;
		log.info(sql);
	...
	public Map<String, Object> getInstanceMap(int instanceId) {
		...
		log.info(sql);
		return this
				.executeStatementReturnsMapWithKeys(sql, "id", "name", "terminated", "scenario_id");
	}
}
