...
@Path("jcomparser") public class REST {
	 ...
	 */
	private static final String PROCESS_SERVER_URI = PropertyLoader
			.getProperty("processeditor.serverUrl");
	 ...
	 */
	@POST @Path("launch/{scenarioID}") public int startComparser(
			@PathParam("scenarioID") final int scenarioID)
	...
	@GET @Path("scenarios/{modelID}/image/") @Produces("image/png") public Response showModelImage(
		...
		return new Retrieval().getImagewithAuth(PROCESS_SERVER_URI, urlToRead);
	}
}
