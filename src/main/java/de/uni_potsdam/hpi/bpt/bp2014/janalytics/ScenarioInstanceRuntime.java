...
public class ScenarioInstanceRuntime implements AnalyticsService {
	 ...
	 */ public JSONObject calculateResult(String[] args) {
		...
		long hour = (duration / (millisecondsPerSecond * secondsPerMinute * minutesPerHour))
				% hoursPerDay;
		long day = (duration / (millisecondsPerSecond * secondsPerMinute * minutesPerHour
				* hoursPerDay)) % daysPerYear;

		String time = String.format("%02d:%02d:%02d:%02d", day, hour, minute, second);
		String json =
				"{\"scenarioId\":" + scenarioInstanceId + ",\"ScenarioInstanceRuntime\":\"" + time
						+ "\"}";
	...
	public void getStartTimestamp(int scenarioInstanceId) {
		...
		String sql =
				"SELECT MIN(timestamp) AS start_timestamp FROM `historydataobjectinstance` as h, scenarioinstance as s WHERE h.scenarioinstance_id = "
						+ scenarioInstanceId + " AND h.scenarioinstance_id = s.id";
