...
public class ScenarioInstance {
	...
	private void initializeFragment(int fragment_id) {
		FragmentInstance fragmentInstance = new FragmentInstance(fragment_id, scenarioInstance_id,
				this);
	...
	private void initializeDataObjects() {
		...
		for (Integer dataObject : data) {
			DataObjectInstance dataObjectInstance = new DataObjectInstance(dataObject, scenario_id,
					scenarioInstance_id, this);
	 ...
	 */
	@SuppressWarnings("unchecked") public void checkExecutingGateways(int controlNode_id) {
		for (GatewayInstance gatewayInstance : ((LinkedList<GatewayInstance>) executingGateways
				.clone())) {
	...
	public boolean checkTerminationCondition() {
		...
		LinkedList<Integer> conditionsSets = dbTerminationCondition
				.getConditionsSetIDsForScenario(scenario_id);
		for (int conditionSet : conditionsSets) {
			LinkedList<Condition> conditions = dbTerminationCondition
					.getConditionsForConditionSetAndScenario(scenario_id, conditionSet);
	...
	@SuppressWarnings("unchecked") public void startAutomaticControlNodes() {
		for (ControlNodeInstance controlNodeInstance : ((LinkedList<ControlNodeInstance>) enabledControlNodeInstances
				.clone())) {
