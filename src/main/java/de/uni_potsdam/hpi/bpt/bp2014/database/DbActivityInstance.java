...
public class DbActivityInstance extends DbObject {
	...
	public void setState(int id, String state) {
		...
		String sql =
				"UPDATE activityinstance SET activity_state = '" + state + "' WHERE id = " + id;
	 ...
	 */
	public int createNewActivityInstance(int controlNodeInstance_id, String ActivityType,
			String ActivityState) {
		String sql =
				"INSERT INTO activityinstance (id, type, role_id, activity_state, workitem_state) VALUES ("
						+ controlNodeInstance_id + ", '" + ActivityType + "', 1,'" + ActivityState
						+ "', 'init')";
	...
	public LinkedList<Integer> getTerminatedActivitiesForFragmentInstance(int fragmentInstance_id) {
		String sql =
				"SELECT controlnode_id FROM activityinstance, controlnodeinstance WHERE activityinstance.id = controlnodeinstance.id AND controlnodeinstance.Type = 'Activity' AND activity_state = 'terminated' AND fragmentinstance_id = "
						+ fragmentInstance_id;
	...
	public LinkedList<Integer> getTerminatedActivitiesForScenarioInstance(int scenarioInstance_id) {
		String sql =
				"SELECT controlnode_id FROM activityinstance, controlnodeinstance WHERE activityinstance.id = controlnodeinstance.id AND controlnodeinstance.Type = 'Activity' AND activity_state = 'terminated' AND fragmentinstance_id IN (Select id FROM fragmentinstance WHERE scenarioinstance_id ="
						+ scenarioInstance_id + ")";
		return this.executeStatementReturnsListInt(sql, "controlnode_id");
	}
	...
	public void setAutomaticExecution(int id, boolean automaticExecution) {
		...
		String sql = "UPDATE activityinstance SET automaticexecution = " + automaticExecutionAsInt
				+ " WHERE id = " + id;
		this.executeUpdateStatement(sql);
	}
	...
	public Map<Integer, Map<String, Object>> getMapForAllActivityInstances(int instanceId) {
		String sql =
				"SELECT activityinstance.id AS id, activityinstance.type AS type, activity_state, "
						+
						...
						+ ")";
		return this.executeStatementReturnsMapWithMapWithKeys(sql, "id", "type", "activity_state",
				"label");
	}
	...
	public Map<Integer, Map<String, Object>> getMapForActivityInstancesWithState(int instanceID,
			String state) {
		String sql =
				"SELECT activityinstance.id AS id, activityinstance.type AS type, activity_state, "
						+
	...
	public Map<Integer, Map<String, Object>> getMapForActivityInstancesWithFilter(int instanceID,
			String filterString) {
		String sql =
				"SELECT activityinstance.id AS id, activityinstance.type AS type, activity_state, "
						+
	...
	public void setCanTerminate(int id, boolean canTerminate) {
		...
		String sql =
				"UPDATE activityinstance SET canTerminate = " + canTerminateAsInt + " WHERE id = "
						+ id;
		this.executeUpdateStatement(sql);
	}
}
