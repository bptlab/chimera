...
public class DataObjectInstance {
	...
	public DataObjectInstance(int dataObject_id, int scenario_id, int scenarioInstance_id,
		...
		if (dbDataObjectInstance.existDataObjectInstance(scenarioInstance_id, dataObject_id)) {
			...
			dataObjectInstance_id = dbDataObjectInstance
					.getDataObjectInstanceID(scenarioInstance_id, dataObject_id);
			...
			this.dataObjectInstance_id = dbDataObjectInstance
					.createNewDataObjectInstance(scenarioInstance_id, state_id, dataObject_id);
		}
		this.initializeAttributes();
	}

	private void initializeAttributes() {
		LinkedList<Integer> dataAttribute_ids = dbDataObject
				.getAllDataAttributesForDataObject(dataObject_id);
		for (int dataAttribute_id : dataAttribute_ids) {
			DataAttributeInstance dataAttributeInstance = new DataAttributeInstance(
					dataAttribute_id, dataObjectInstance_id, this);
			dataAttributeInstances.add(dataAttributeInstance);
			scenarioInstance.getDataAttributeInstances()
					.put(dataAttributeInstance.getDataAttributeInstance_id(),
