...
public class DbControlNode extends DbObject {
	...
	public int getStartEventID(int fragment_id) {
		String sql = "SELECT id FROM controlnode WHERE type = 'Startevent' AND fragment_id = "
				+ fragment_id;
		log.info(sql);
	...
	public String getType(int controlNode_id) {
		if (type == null || this.controlNode_id != controlNode_id) {
			...
			log.info("Used cached type instead of querying the database for contolNodeId="
					+ controlNode_id);
		}
		return type;
	}
		...
		try {
			...
			String sql =
					"(SELECT dataobject_id, state_id FROM datasetconsistsofdatanode, datanode, dataflow "
							...
							+
							"datasetconsistsofdatanode.datanode_id = datanode.id AND controlnode_id = "
							+ controlNode_id1 + " AND " +
