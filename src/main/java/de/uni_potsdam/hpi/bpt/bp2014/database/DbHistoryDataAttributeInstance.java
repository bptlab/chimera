...
public class DbHistoryDataAttributeInstance extends DbObject {
    ...
    public int createEntry(int data_attribute_instance_id, Object value) {
        String sql = "INSERT INTO `historydataattributeinstance` (`scenarioinstance_id`,`dataattributeinstance_id`,`oldvalue`,`newvalue`) " +
                "SELECT (SELECT `scenarioinstance_id` FROM `dataattributeinstance` WHERE `id` = "+data_attribute_instance_id+") AS`scenarioinstance_id`," +
                " `id`," +
                "(SELECT `value` FROM `dataattributinstance` WHERE `id` = "+data_attribute_instance_id+") AS `oldvalue`, " +
    ...
    public int createNewDataAttributeInstanceEntry (int data_attribute_instance_id){
        String sql = "INSERT INTO `historydataattributeinstance` (`scenarioinstance_id`,`dataattributeinstance_id`,`newvalue`) " +
                "SELECT (SELECT `scenarioinstance_id` FROM `dataattributeinstance` WHERE `id` = "+data_attribute_instance_id+") AS`scenarioinstance_id`," +
                " `id`," +
                "(SELECT `value` FROM `dataattributinstance` WHERE `id` = "+data_attribute_instance_id+") AS `newvalue`, " +
                "FROM `dataattributeinstance` WHERE `id` = "+data_attribute_instance_id;
        return this.executeInsertStatement(sql);
    }
    ...
    public Map<Integer, Map<String, Object>> getLogEntriesForScenarioInstance(int scenarioInstanceId){
        String sql ="SELECT h.id, h.scenarioinstance_id, h.timestamp, h.oldvalue, h.newvalue, h.dataattributeinstace_id, da.name, do.name FROM historydataattributeinstance AS h, dataattributeinstance AS dai, dataattribute AS da, dataobjectinstance AS doi, dataobject AS do WHERE scenarioinstance_id = "+scenarioInstanceId+" AND h.dataattributeinstance = dai.id AND dai.dataattribute_id = da.id AND dai.dataobjectinstance_id = doi.id AND doi.dataobject_id = do.id ORDER BY timestamp DESC";
