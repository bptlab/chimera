...
public class FragmentValidator {
	...
	private static void validateNames(Fragment fragment) {
		...
		for (AbstractDataControlNode task : fragment.getTasks()) {
			if (!compiledPattern.matcher(task.getName()).matches() && !"".equals(task.getName())) {
				throw new IllegalArgumentException(String.format("%s is not a valid task name", task.getName()));
			}
		}
		for (DataNode dataNode : fragment.getDataNodes()) {
			if (!compiledPattern.matcher(dataNode.getName()).matches() && !"".equals(dataNode.getName())) {
				throw new IllegalArgumentException(String.format("%s is not a valid data node name", dataNode.getName()));
			}
		}
	}
	...
	private static void validateOlc(Map<String, Olc> olcs, Fragment fragment) {
		...
		for (AbstractDataControlNode task : fragment.getAllActivities()) {
			Map<String, List<String>> incomingDataobjectStates = getIncomingStatesPerDataobject(task, idToDataNode);
			Map<String, List<String>> outgoingDataobjectStates = getOutgoingStatesPerDataobject(task, idToDataNode);
			List<String> dataObjectsNames = getDataObjectsToBeChecked(incomingDataobjectStates.keySet(), outgoingDataobjectStates.keySet(), olcs.keySet());
			...
			for (String dataobjectName : dataObjectsNames) {
				...
				/* This is the case when dataobjects are created (i.e., not in input, but in output)
					...
					for (String state : inputStates) {
						...
						List<String> allowedTransitionsForState = olcForDataobject.allowedStateTransitions.get(state);
	...
	private static void validateDataReferences(Fragment fragment, DomainModel domainModel) {
		...
		for (DataNode node : fragment.getDataNodes()) {
			if (!dataclassNames.contains(node.getName())) {
				throw new InvalidDataclassReferenceException(String.format("Data node %s references an invalid data class", node.getName()));
			}
		}
	}
	...
	private static Map<String, List<String>> getIncomingStatesPerDataobject(AbstractDataControlNode task, Map<String, DataNode> idToDataNode) {
	...
	private static Map<String, List<String>> getOutgoingStatesPerDataobject(AbstractDataControlNode task, Map<String, DataNode> idToDataNode) {
