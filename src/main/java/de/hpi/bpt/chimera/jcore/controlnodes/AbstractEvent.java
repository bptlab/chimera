...
public abstract class AbstractEvent extends AbstractControlNodeInstance {
	 ...
	 */
	public AbstractEvent(int controlNodeId, int fragmentInstanceId, ScenarioInstance scenarioInstance) {
		...
		if (!databaseNodeInstance.existControlNodeInstance(controlNodeId, fragmentInstanceId)) {
			int controlNodeInstanceId = databaseNodeInstance.createNewControlNodeInstance(controlNodeId, this.getType(), fragmentInstanceId, State.INIT);
			this.setControlNodeInstanceId(controlNodeInstanceId);
			this.setState(State.INIT);
		} else {
			this.setControlNodeInstanceId(databaseNodeInstance.getControlNodeInstanceId(controlNodeId, fragmentInstanceId));
			this.setState(databaseNodeInstance.getState(this.getControlNodeInstanceId()));
		}
		outgoingBehavior = this.createOutgoingBehavior();
	}

	public AbstractEvent(int controlNodeId, int fragmentInstanceId, ScenarioInstance scenarioInstance, int controlNodeInstanceId) {
		this.setControlNodeId(controlNodeId);
		this.scenarioInstance = scenarioInstance;
		this.setFragmentInstanceId(fragmentInstanceId);
		this.setExecutionBehavior(new EventExecutionBehavior(this));
	...
	protected EventOutgoingBehavior createOutgoingBehavior() {
		return new EventOutgoingBehavior(this.getControlNodeId(), scenarioInstance, getFragmentInstanceId(), getControlNodeInstanceId());
	}
