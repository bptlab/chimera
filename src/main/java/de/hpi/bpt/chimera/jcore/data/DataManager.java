...
public class DataManager {
	...
	public Boolean changeDataObjectState(int dataObjectId, int stateId, int activityInstanceId) {
		...
		if (dataObject.isPresent()) {
			...
			new DbLogEntry().logDataObjectTransition(dataobjectId, stateId, activityInstanceId, this.scenarioInstance.getId());
			return true;
		}
		return false;
	}
	...
	public void loadFromDatabase() {
		...
		List<Integer> dataObjectIds = dbDataObject.getDataObjectIds(scenarioInstance.getId());
	...
	public Optional<DataObject> getDataObjectForId(int dataObjectId) {
		return this.dataObjects.stream().filter(x -> x.getId() == dataObjectId).findFirst();
	}
	...
	public Boolean lockDataobject(int dataObjectId) {
		Optional<DataObject> dataObject = this.getDataObjectForId(dataObjectId);
	...
	public Map<Integer, Integer> translate(Map<String, String> dataClassNameToStateName) {
		...
		for (Map.Entry<String, String> entry : dataClassNameToStateName.entrySet()) {
			...
			dataClassIdToStateId.put(dataClassId, dbState.getStateId(dataClassId, entry.getValue()));
		}
	...
	public boolean setAttributeValues(int activityInstanceId, Map<Integer, String> idToValue) {
		...
		for (Map.Entry<Integer, String> attributeInstanceIdToValue : idToValue.entrySet()) {
			...
			if (dataAttributeInstance.isValueAllowed(value)) {
				...
				log.error("Attribute value could not be set " + "because it has the wrong data type.");
				SseNotifier.notifyError("Attribute value could nto be set " + "because it has the wrong data type.");
	...
	private void updateDataObjectAttribute(DataAttributeInstance attributeInstance) {
		...
		Optional<DataObject> object = getDataObjectForId(db.getDataObjectId(attributeInstanceId));
		if (object.isPresent()) {
