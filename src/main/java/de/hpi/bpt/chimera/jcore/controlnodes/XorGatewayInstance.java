...
public class XorGatewayInstance extends GatewayInstance {
	...
	public XorGatewayInstance(int controlNodeId, int fragmentInstanceId, ScenarioInstance scenarioInstance) {
		...
		this.setControlNodeInstanceId(dbControlNodeInstance.createNewControlNodeInstance(controlNodeId, "XOR", fragmentInstanceId, State.INIT));
		new Connector().insertGatewayInstance(getControlNodeInstanceId(), "XOR", "init");
		this.initGatewayInstance();
	}

	public XorGatewayInstance(int controlNodeId, int fragmentInstanceId, ScenarioInstance scenarioInstance, int instanceId) {
	...
	private void initGatewayInstance() {
		this.setOutgoingBehavior(new ExclusiveGatewaySplitBehavior(getControlNodeId(), scenarioInstance, getFragmentInstanceId()));
		this.setIncomingBehavior(new ExclusiveGatewayJoinBehavior(this, scenarioInstance));
	}

	public boolean containsControlNodeInFollowing(int controlNodeId) {
		ExclusiveGatewaySplitBehavior outgoing = (ExclusiveGatewaySplitBehavior) this.getOutgoingBehavior();
