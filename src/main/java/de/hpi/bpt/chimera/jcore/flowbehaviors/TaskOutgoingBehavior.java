...
public class TaskOutgoingBehavior extends AbstractParallelOutgoingBehavior {
	 ...
	 */
	public TaskOutgoingBehavior(int activityId, ScenarioInstance scenarioInstance, int fragmentInstanceId, ActivityInstance activityInstance) {
	...
	@Override
	public void skip() {
		new DbLogEntry().logActivity(this.activityInstance.getControlNodeInstanceId(), "skipped", this.getScenarioInstance().getId());
	}
	...
	public void terminate(Map<String, String> dataClassNameToStateName) {
		...
		List<DataObject> toUpdate = usedDataObjects.stream().filter(x -> outputClassIds.contains(x.getDataClassId())).collect(Collectors.toList());
	...
	private void createDataObjects(Set<Integer> toCreate, Map<String, String> dataClassNameToStateName) {
		...
		Map<Integer, Integer> dataClassIdToStateId = dataManager.translate(dataClassNameToStateName);
	...
	private void updateDataObjects(Map<String, String> dataClassNameToStateName, List<DataObject> toUpdate) {
		DataManager dataManager = this.getScenarioInstance().getDataManager();
		Map<Integer, Integer> dataClassIdToStateId = translate(this.getScenarioInstance().getScenarioId(), dataClassNameToStateName);
		...
		Set<Integer> usedDataclassIds = toUpdate.stream().map(DataObject::getDataClassId).collect(Collectors.toSet());
	...
	private Map<String, String> loadOnlyOutputSet() {
		...
		if (outputSets.size() > 1) {
			throw new IllegalArgumentException("Should only be used when there are no " + "alternative output sets.");
		}
	...
	private Map<Integer, Integer> getClassToSelectedObjectIdMap() {
		...
		int scenarioInstanceId = this.getScenarioInstance().getId();
		List<Integer> dataObjectSelection = dbSelection.getDataObjectSelection(scenarioInstanceId, controlNodeInstanceId);
		return dataObjectSelection.stream().collect(Collectors.toMap(dbDataObject::getDataClassId, Function.identity()));
	}
	 ...
	 * @return
	 */
	private Map<Integer, Integer> translate(int scenarioId, Map<String, String> dataClassNameToStateName) {
	...
	private List<DataObject> getUsedDataobjects() {
		...
		Set<Integer> workItems = new HashSet<>(dbSelectedDataObjects.getDataObjectSelection(instance.getId(), this.activityInstance.getControlNodeInstanceId()));
		List<DataObject> dataObjects = instance.getDataManager().getDataObjects();
		return dataObjects.stream().filter(x -> workItems.contains(x.getId())).collect(Collectors.toList());
	}
	...
	public void cancel() {
		DbSelectedDataObjects dbSelectedDataObjects = new DbSelectedDataObjects();
		List<Integer> usedDataObjects = dbSelectedDataObjects.getDataObjectSelection(this.getScenarioInstance().getId(), this.getControlNodeId());
	...
	public Boolean changeDataObjectState(int dataObjectId, int stateId) {
		DataManager dataManager = this.getScenarioInstance().getDataManager();
		return dataManager.changeDataObjectState(dataObjectId, stateId, activityInstance.getControlNodeInstanceId());
	}
	...
	public void unlockDataObject(int dataObjectId) {
		DataManager dataManager = this.getScenarioInstance().getDataManager();
		Optional<DataObject> dataObjectInstance = dataManager.getDataObjectForId(dataObjectId);
