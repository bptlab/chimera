...
public class EventBasedGatewayInstance extends GatewayInstance {
	...
	public EventBasedGatewayInstance(int controlNodeId, int fragmentInstanceId, ScenarioInstance scenarioInstance) {
		...
		this.setControlNodeInstanceId(dbControlNodeInstance.createNewControlNodeInstance(controlNodeId, "EventBasedGateway", fragmentInstanceId, State.INIT));
		new Connector().insertGatewayInstance(getControlNodeInstanceId(), "EventBasedGateway", "init");
	}

	public EventBasedGatewayInstance(int controlNodeId, int fragmentInstanceId, ScenarioInstance scenarioInstance, int instanceId) {
	...
	public void terminate() {
		...
		List<Integer> followingControlNodes = controlFlow.getFollowingControlNodes(this.getControlNodeId());
		EventFactory factory = new EventFactory(this.scenarioInstance);
		List<AbstractEvent> followingEvents = followingControlNodes.stream().map(x -> factory.getEventForControlNodeId(x, this.getFragmentInstanceId())).collect(Collectors.toList());
