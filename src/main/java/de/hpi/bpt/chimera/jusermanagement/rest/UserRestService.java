package de.hpi.bpt.chimera.jusermanagement.rest;

import com.google.gson.Gson;
import de.hpi.bpt.chimera.jusermanagement.Controller;
...
public class UserRestService {
	...
	public Response getAllUser(@QueryParam("filter") String filterString) {
		String jsonRepresentation = toJson(Controller.retrieveAllItems("um_user"));
	...
	@Path("user/{userID}")
	@Produces(MediaType.APPLICATION_JSON)
	public Response getSpecificUser(@QueryParam("filter") String filterString, @PathParam("userID") int userID) {
		String jsonRepresentation = toJson(Controller.retrieveItem("um_user", userID));
		return Response.ok(jsonRepresentation, MediaType.APPLICATION_JSON).build();
	}
	...
	public Response getAllRoles(@QueryParam("filter") String filterString) {
		String jsonRepresentation = toJson(Controller.retrieveAllItems("um_role"));
		return Response.ok(jsonRepresentation, MediaType.APPLICATION_JSON).build();
	}
	...
	@Path("role/{roleID}")
	@Produces(MediaType.APPLICATION_JSON)
	public Response getSpecificRole(@QueryParam("filter") String filterString, @PathParam("roleID") int roleID) {
		String jsonRepresentation = toJson(Controller.retrieveItem("um_role", roleID));
		return Response.ok(jsonRepresentation, MediaType.APPLICATION_JSON).build();
	}
	...
	public Response deleteUser(@PathParam("userID") Integer userID) throws Exception {
		...
		if (result) {
			return Response.status(Response.Status.ACCEPTED).type(MediaType.APPLICATION_JSON).entity("{\"message\":\"user deletion successfully.\"}").build();
		} else {
			return Response.status(Response.Status.BAD_REQUEST).type(MediaType.APPLICATION_JSON).entity("{\"error\":\"user deletion failed\"}").build();
		}
	}
	...
	@Produces(MediaType.APPLICATION_JSON)
	public Response createNewRole(@Context UriInfo uriInfo, NamedJaxBean name) {

		int roleId = Controller.createNewRole(name.name, name.description, name.admin_id);
		return Response.status(Response.Status.CREATED).type(MediaType.APPLICATION_JSON).entity("{\"id\":" + roleId + ",\"link\":\"" + uriInfo.getAbsolutePath() + "/" + roleId + "\"}").build();
	}
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response updateRoleSpecs(@Context UriInfo uriInfo, @PathParam("roleID") int roleID, NamedJaxBean name) {

		boolean result = Controller.updateRole(roleID, name.name, name.description, name.admin_id);

		if (!result) {
			return Response.status(Response.Status.BAD_REQUEST).type(MediaType.APPLICATION_JSON).entity("{\"error\":\"The Scenario instance could not be found!\"}").build();

		} else {
			return Response.status(Response.Status.OK).type(MediaType.APPLICATION_JSON).entity("{\"message\":\"The user details were updated.\"}").build();
		}
	}
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response createNewUser(@Context UriInfo uriInfo, NamedJaxBean name) {

		int userId = Controller.createNewUser(name.name, name.role_id, name.description);

		return Response.status(Response.Status.CREATED).type(MediaType.APPLICATION_JSON).entity("{\"id\":" + userId + ",\"link\":\"" + uriInfo.getAbsolutePath() + "/" + userId + "\"}").build();
	}
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response updateUserSpecs(@Context UriInfo uriInfo, @PathParam("userID") int userID, NamedJaxBean name) {
