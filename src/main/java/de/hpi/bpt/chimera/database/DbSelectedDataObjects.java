...
public class DbSelectedDataObjects extends DbObject {
	 ...
	 */
	public void saveDataObjectSelection(int scenarioInstanceId, int activityInstanceId, List<Integer> dataObjectIds) {
		String insertTemplate = "INSERT INTO dataobjectselection " + "(scenarioinstance_id, activityinstance_id, dataobject_id) VALUES " + "(%d, %d, %d);";
		for (Integer dataObjectId : dataObjectIds) {
			String sql = String.format(insertTemplate, scenarioInstanceId, activityInstanceId, dataObjectId);
	...
	public List<Integer> getDataObjectSelection(int scenarioInstanceId, int activityInstanceId) {
		String getWorkingItems = "Select * FROM dataobjectselection WHERE scenarioinstance_id = %d AND " + "activityinstance_id = %d;";
