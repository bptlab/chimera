...
public class DbLogEntry extends DbObject {
	...
	private final static String insertQuery = "INSERT INTO logentry (`scenarioinstance_id`, `logged_id`," + "`new_value`, `cause`, `label`, `type`, `timestamp`) VALUES " + "(?, ?, ?, ?, ?, ?, ?);";
	...
	public void logActivity(int activityInstanceId, String state, int scenarioInstanceId) {
		...
		this.insertLog(scenarioInstanceId, activityInstanceId, state, Optional.empty(), label, LogEntry.LogType.ACTIVITY);
	}
	 ...
	 */
	public void logDataAttributeTransition(int dataAttributeInstanceId, Object value, int causeInstanceId, int scenarioInstanceId) {
		...
		this.insertLog(scenarioInstanceId, dataAttributeInstanceId, value.toString(), Optional.of(causeInstanceId), label, LogEntry.LogType.DATA_ATTRIBUTE);
	}
	 ...
	 */
	public void logDataAttributeTransition(int dataAttributeInstanceId, Object value, int scenarioInstanceId) {
		DbDataAttributeInstance attributeDao = new DbDataAttributeInstance();
		int dataAttributeId = attributeDao.getDataAttributeID(dataAttributeInstanceId);
		String label = attributeDao.getName(dataAttributeId);
		this.insertLog(scenarioInstanceId, dataAttributeInstanceId, value.toString(), Optional.empty(), label, LogEntry.LogType.DATA_ATTRIBUTE);
	}
	 ...
	 */
	public void logDataObjectTransition(int objectInstanceId, int stateId, int activityInstanceId, int scenarioInstanceId) {
	...
	public void logDataObjectCreation(int objectInstanceId, int stateId, int scenarioInstanceId) {
		...
		String label = new DbDataClass().getName(dataObjectId);
		String state = new DbState().getStateName(stateId);
		this.insertLog(scenarioInstanceId, objectInstanceId, state, Optional.empty(), label, LogEntry.LogType.DATA_OBJECT);
	}

	public void logDataAttributeCreation(int dataAttributeInstanceId, Object value, int scenarioInstanceId) {
		DbDataAttributeInstance attributeDao = new DbDataAttributeInstance();
		int dataAttributeId = attributeDao.getDataAttributeID(dataAttributeInstanceId);
		String label = attributeDao.getName(dataAttributeId);
		if (null == value) {
	...
	public void logEvent(int eventInstanceId, int scenarioInstanceId, String status) {
		...
		this.insertLog(scenarioInstanceId, eventInstanceId, status, Optional.empty(), label, LogEntry.LogType.EVENT);
	}
	 ...
	 */
	public List<LogEntry> getLogEntriesForScenarioInstance(int scenarioInstanceId, LogEntry.LogType type) {
		String sql = "SELECT * FROM logentry WHERE scenarioinstance_id = %d AND type = '%s' " + "ORDER BY timestamp ASC;";
	...
	public List<LogEntry> getLogEntriesForScenarioInstance(int scenarioInstanceId) {
		String sql = "SELECT * FROM logentry WHERE scenarioinstance_id = %d " + "ORDER BY timestamp ASC;";
	...
	public List<LogEntry> getCreationLogEntries(int scenarioInstanceId) {
		String sql = "SELECT l1.* FROM logentry l1 " + "INNER JOIN (SELECT logged_id, MIN(timestamp) as timestamp FROM logentry GROUP BY logged_id) l2 " + "ON l1.logged_id = l2.logged_id WHERE l1.timestamp = l2.timestamp " + "AND l1.scenarioinstance_id = %d;";
	...
	public List<LogEntry> getCreationLogEntries(int scenarioInstanceId, LogEntry.LogType type) {
		String sql = "SELECT l1.* FROM logentry l1 " + "INNER JOIN (SELECT logged_id, MIN(timestamp) as timestamp, type FROM logentry " + "GROUP BY logged_id, type) l2 " + "ON l1.logged_id = l2.logged_id WHERE l1.timestamp = l2.timestamp AND " + "l1.type = l2.type AND l1.scenarioinstance_id = %d AND l1.type = '%s';";
	...
	private List<LogEntry> retrieveLogEntries(String sql) {
		...
		try (java.sql.Connection conn = ConnectionWrapper.getInstance().connect(); Statement statement = conn.createStatement(); ResultSet rs = statement.executeQuery(sql)) {
			while (rs.next()) {
	...
	private void insertLog(int scenarioInstanceId, int loggedId, String newValue, Optional<Integer> cause, String label, LogEntry.LogType type) {
