...
public class DbActivityInstance extends DbObject {
	...
	public int createNewActivityInstance(int controlNodeInstanceId, String activityType) {
		...
		String sql = "Insert INTO activityinstance (id, type, role_id, workitem_state) " + "VALUES (%d, '%s', 1, 'init')";
	...
	public Boolean existsActivityInstance(int controlNodeInstanceId, int scenarioInstanceId) {
		String existsActivityInstance = "SELECT * FROM controlnodeinstance as cni, " + "fragmentinstance as fi WHERE cni.fragmentinstance_id = fi.id " + "AND fi.scenarioinstance_id = %d AND cni.id = %d;";
		existsActivityInstance = String.format(existsActivityInstance, scenarioInstanceId, controlNodeInstanceId);
	 ...
	 */
	public List<Integer> getTerminatedActivitiesForScenarioInstance(int scenarioInstanceId) {
		String sql = "SELECT controlnode_id " + "FROM activityinstance, controlnodeinstance " + "WHERE " + "activityinstance.id = controlnodeinstance.id " + "AND controlnodeinstance.Type = 'Activity' " + "AND controlnodeinstance.state = 'terminated' " + "AND fragmentinstance_id " + "IN (SELECT id FROM fragmentinstance " + "WHERE scenarioinstance_id =" + scenarioInstanceId + ")";
	...
	public String getLabel(int activityInstanceId) {
		String sql = "SELECT cn.label as label FROM controlnodeinstance as cni," + "controlnode as cn WHERE cni.controlnode_id = cn.id AND cni.id = %d; ";
