...
public class DbDataFlow extends DbObject {
	...
	public List<Integer> getInputSetsForControlNode(int controlNodeId) {
		String sql = "Select dataset_id FROM dataflow " + "WHERE dataflow.input = 1 " + "AND controlnode_id = " + controlNodeId;
	...
	public List<Integer> getOutputSetsForControlNode(int controlNodeId) {
		String sql = "Select dataset_id FROM dataflow " + "WHERE dataflow.input = 0 " + "AND controlnode_id = " + controlNodeId;
		return this.executeStatementReturnsListInt(sql, "dataset_id");
	}
	...
	public int getPrecedingControlNode(int dataNodeId) {
		String sql = "SELECT d.controlnode_id as cn_id " + "FROM datanode as n, datasetconsistsofdatanode as s, dataflow as d " + "WHERE n.id = s.datanode_id AND d.dataset_id = s.dataset_id " + "AND d.input = 0 AND n.id = %d;";
	...
	private List<Integer> getDataClassIds(int input, int controlNodeId) {
		String sql = "SELECT n.dataclass_id as class_id " + "FROM datanode as n, dataflow as f, datasetconsistsofdatanode as s " + "WHERE f.input = %d " + "AND f.controlnode_id = %d " + "AND f.dataset_id = s.dataset_id " + "AND n.id = s.datanode_id";
		return this.executeStatementReturnsListInt(String.format(sql, input, controlNodeId), "class_id");
	}
}
