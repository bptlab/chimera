...
public class FragmentInserter {
	...
	private void saveBoundaryEventRelations(Map<String, Integer> savedControlNodes, Fragment fragment, int fragmentId) {
	...
	private void savePathMapping(Fragment fragment, List<DataClass> dataClasses) {
		for (DataNode dataNode : fragment.getDataNodes()) {
			int dataClassId = new DbDataNode().getDataClassIdForDataNode(dataNode.getDatabaseId());
