...
public class TerminationCondition {
	 ...
	 */
	static List<TerminationCondition> parseTerminationConditions(JSONArray jsonTerminationConditions, List<DataClass> dataClasses, int scenarioId, Map<String, Integer> stateToDatabaseId) {
		...
		for (int i = 0; i < jsonTerminationConditions.length(); i++) {
			...
			conditions.add(parseTerminationConditionString(condition, dataClasses, scenarioId, stateToDatabaseId));
		}
		return conditions;
	}
	 ...
	 */
	private static TerminationCondition parseTerminationConditionString(String conditions, List<DataClass> dataClasses, int scenarioId, Map<String, Integer> stateToDatabaseId) {
		TerminationCondition condition = new TerminationCondition(scenarioId);
		Map<String, Integer> dataClassNameToId = dataClasses.stream().collect(Collectors.toMap(DataClass::getName, DataClass::getDatabaseId));
	...
	private static Matcher findTerminationCondition(String terminationCondition) {
		...
		if (match.find()) {
			...
			throw new IllegalArgumentException(String.format("Malformed termination String %s", terminationCondition));
		}
	}
	...
	public void save() {
		...
		for (Map.Entry<Integer, Integer> condition : this.dataObjectIdToStateId.entrySet()) {
			conn.insertTerminationCondition(condition.getKey(), condition.getValue(), scenarioId, conditionSetId);
		}
	}
